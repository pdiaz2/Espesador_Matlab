...
if generateOne
    ...
    pause(1)
    %% Generate RF Model
    [resultsIter,ML_Models,trainingTime] = ml_model_generation(trainingBigSet,...
                                                        testBigSet,...
                                                        controlParamsStruct,...
                                                        mlParamsStruct,...
                                                        mOrder);
    %%
    RF_Ensemble = ML_Models;
    for forest = 1:size(RF_Ensemble,2)
        ML_Model(forest).Model = compact(RF_Ensemble(forest).Model);
    end
    RF = RF_Ensemble(forestToView).Model;
...
%                 print(printName,'-djpeg');
        ...
        outputmatFileName = [predictiveModelsPath ...
                            ...
                            ioDTStr noiseStr...
                            'B' num2str(numTrees) '_Ny_' num2str(N_y) ...
                            '_k' num2str(controlParamsStruct.tau_R) '_'...
                            'na' num2str(delayYCases) '_nb' num2str(delayUCases)...
                            '_' dateTest];
