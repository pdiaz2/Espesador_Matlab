function [ IOData,delayMaxInTime] = Prepare_IO_Data( numInputs,numOutputs,effectiveReactionTime,...
...
if (strcmp(mlMethod,'RF'))
    ...
    Y = [];
    for u = 1:numInputs
        U = [U BigData.Inputs.TimeSeries(:,u)];
    end
    for y = 1:numOutputs
        Y = [Y BigData.Outputs.TimeSeries(:,y)];
    end
...
elseif (strcmp(mlMethod,'SS')||strcmp(mlMethod,'ARMAX'))
    U = BigData.Inputs.TimeSeries(:,:);
    Y = BigData.Outputs.TimeSeries(:,:);
    Ts = YPastValues;
