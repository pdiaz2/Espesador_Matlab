...
%   Detailed explanation goes here
    ...
    for t = 1:numOLExps
        ...
        OutputTimeSeries = [OutputTimeSeries;YExp(1+delayMaxInTime+deadTimeMax+jAhead:end,:)];
    end
    for cv = 1:n
        ...
        U_CV = UPastSequence(1+jAhead:end,1+(cv-1)*m:cv*m);
        arxRFSet(cv).InputData = [YPastSequence(1:end-jAhead,1+startIndexY:endIndexY) U_CV];
