...
%         indicesMV = (1+indicesDV(end))+cumDelayMV:(1+indicesDV(end))+(sum(nb(cv,1:(mv)))-1);
        ...
        computedMV = compute_MV_value(x,uPastValues,mv,m,jAhead,N_u,NUM_PSEUDO_COSTS);  
        MVVector(:,1) = computedMV;
        if delayU >= 1 % Change to 2 after new RF Model
            MVVector(:,2:end) = uPastValues(:,[1:delayU],mv); %Works even if delayU = 0. After new RF Model, delayU >=1
        end
