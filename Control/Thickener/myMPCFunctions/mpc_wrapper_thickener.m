function [ optimalMove,fval,exitflag,population,scores,optimalMVSequence,yHat ]...
                                                            = mpc_wrapper_thickener...
                                                            ...
                                                            OptimSolverParameters,controlHit)
...
elseif OptimSolverParameters(end-1) == 2
    ...
    outputFcn = @(optimValues, state)mpc_output_pso_fcn(optimValues, state, controlHit,...
                        qMatrix, betaCost, lambdaMatrix, wMatrix, yLims,uLims,...
                        yPastValues, uPastValues, dPastValues,...
                        nTrees, nPredictors, na, nb, nc, kappaControl,...
                        OptimSolverParameters(end-3),OptimSolverParameters(end-4),...
                        OptimSolverParameters(6),OptimSolverParameters(7));
...
elseif OptimSolverParameters(end-1) == 3
    outputFcn = @(stop, options, optimvalues)mpc_output_patts_fcn(flag, options, optimvalues, ...
                                    qMatrix, betaCost, lambdaMatrix, wMatrix,...
                                    yLims,uLims,...
                                    yPastValues, uPastValues, dPastValues,...
                                    nTrees, nPredictors,...
                                    na, nb, nc, kappaControl,...
                                    OptimSolverParameters(end-3));%, championTolerance,...
                                    %championCounterTolerance);
    firstOptions = optimoptions('patternsearch','UseVectorized',true,'Display','off',...
                    'MaxFunctionEvaluations',OptimSolverParameters(1),...
                    'MaxIterations',OptimSolverParameters(2),...
                    'FunctionTolerance',OptimSolverParameters(3),...
                    'StepTolerance',OptimSolverParameters(4),...
                    'OutputFcn',[]...
                    );
end
...
elseif OptimSolverParameters(end-1) == 3
    [x,fval,exitflag,output] = ...
        patternsearch(objectiveFunction,zeros(1,OptimSolverParameters(end)),...
...
% Optimal MV calculated
optimalMove = mpc_saturate_mv(uPossible,[uLowLims uHighLims]);
%% Projected Trajectory
[yHat,optimalMVSequence] = mpc_optimization_solutions( x,...
                                                       yPastValues,uPastValues,dPastValues,...
                                                       n,m,N_y,N_u,...
                                                       nTrees,nPredictors,...
                                                       na,nb,nc,kappaControl,...
