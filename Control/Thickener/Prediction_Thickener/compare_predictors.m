...
for cv = 1:n
    ...
    results = ml_validate_compact_model(testSubsetRF,ML_Model,...
    ...
    yForecast = ml_forecast(ML_Model,testSubsetRF(cv).InputData(1+pastDataSamples:end,:),...
...
if strcmp(typeOfData,'Sim_')
    ...
    for simIter = 1:size(makeStepMatrix)
        ...
        for cv = 1:3
            ...
            yRC_RF(cv,:) = ml_forecast_step(ML_Model,inputTimeSeries,...
