close all;
imprint = true;
figurePath = 'figures\FinalSim\paper\';
...
for iter = 1:length(kAheadIter)
    ...
    %     title(CVNames{cv})
        ...
        hold off
        grid on
        legendCell = {'Validation Data','ARIMAX Prediction','Forest Prediction'};
        locationStr = input('Loc','s');
        location = locateLegends(locationStr);
        legend(legendCell,'Location',location);
...
load('predictionResults_18G_920_B100_k5_na6_nb6');
kAheadIter = 18;
for iter = 1:length(kAheadIter)
    for cv = 1:n
        figure
    %     title(CVNames{cv})
        ...
        location = locateLegends(locationStr);
        legend(legendCell,'Location',location);
        printName = [figurePath 'predict_18_' CVSaveName{cv}];
        if imprint
            print(printName,'-depsc');
        end
    end
